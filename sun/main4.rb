...
class Turret
  ...
  def initialize(window)
    ...
    @damping_max = 0.999
    @damping_min = 0.0001
    @gravitational_angle = 180
    @gravitational_angle_delta = 2
    @gravitational_angle_max = 360
    @gravitational_angle_min = 0
    @angle = 45
    @angle_delta = 1
    @angle_max = 90
    @angle_min = 0
  ...
  def increase_gravity
    ...
    @gravity = [@gravity, @gravity_max].min
  end
  def decrease_gravity
    @gravity -= @gravity_delta
    @gravity = [@gravity, @gravity_min].max
  end

  def increase_gravitational_angle
    @gravitational_angle += @gravitational_angle_delta
    @gravitational_angle = [@gravitational_angle, @gravitational_angle_max].min
  end
  def decrease_gravitational_angle
    @gravitational_angle -= @gravitational_angle_delta
    @gravitational_angle = [@gravitational_angle, @gravitational_angle_min].max
  end

  def gravitation_vector_unit
    ga = (@gravitational_angle + 270) % 360
    v = (ga * Math::PI/180.0).radians_to_vec2
#    v.y *= -1
    v
  end
...
class GameWindow < Gosu::Window
  ...
  def update
    ...
    total_gravity = @player.turret.gravitation_vector_unit * @player.turret.gravity
    @space.gravity = total_gravity
  ...
  def draw
    ...
    @font.draw("Gravity: #{@space.gravity} Damping: #{@space.damping} ", 10, 70, ZOrder::UI, 1.0, 1.0, 0xffffff00)
    @font.draw("Gravitational Angle: #{@player.turret.gravitational_angle} Vector: #{@player.turret.gravitation_vector_unit} ", 10, 90, ZOrder::UI, 1.0, 1.0, 0xffffff00)
  end
