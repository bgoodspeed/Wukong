...
Feature: Animation Details
  ...
  Scenario: Animation Stopping
    Given I load the game on level "trivial" with screen size 640, 480
    And I create an animation controller with a ratio of one animation tick to 1 game ticks
    And I set the player avatar to "avatar.bmp"
    And I set the game clock to 60 fps
    When I run the game loop 1 times
    Then the player main animation should not be active
    Then the player main animation should not need an update
    Then the player main animation should be on tick 0
    Then the player main animation should be on frame 0

  Scenario: Animation Starting During Movement
    Given I load the game on level "trivial" with screen size 640, 480
    And I create an animation controller with a ratio of one animation tick to 1 game ticks
    And I set the player avatar to "avatar.bmp"
    And I set the game clock to 60 fps
    ...
    Then the player main animation should not be active
    Then the player main animation should be on tick 2
    Then the player main animation should be on frame 0
